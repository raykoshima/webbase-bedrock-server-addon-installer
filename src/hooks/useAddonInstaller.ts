...
export function useAddonInstaller(): UseAddonInstallerReturn {
    ...
    const importAddonFiles = useCallback(async (files: File[]) => {
        ...
        if (validFiles.length === 0) {
            setError('No valid addon files selected. Please select .mcpack or .mcaddon files.');
            return;
        }
        ...
        try {
            ...
            for (const [uuid, filenames] of uuidToFiles) {
                if (filenames.length > 1) {
                    batchDuplicates.push(`"${filenames.join('" and "')}" (same pack UUID)`);
                }
            }

            if (batchDuplicates.length > 0) {
                setError(`Duplicate packs detected in uploaded files: ${batchDuplicates.join('; ')}`);
                return;
            }
            ...
            setPendingPacks(prev => {
                const existingUuids = new Map(prev.map(p => [p.manifest.header.uuid, p.originalFileName]));
