...
export async function installPack(
    ...
    try {
        ...
        const targetDirName = pack.packType === 'behavior' ? 'behavior_packs' : 'resource_packs';
...
async function registerPackInWorld(
    ...
    const jsonFileName = pack.packType === 'behavior'
    ...
    const existingIndex = entries.findIndex(e => e.pack_id === pack.manifest.header.uuid);
    ...
    const encoder = new TextEncoder();
    await writeFile(worldDirHandle, jsonFileName, encoder.encode(JSON.stringify(entries, null, 2)));
}
